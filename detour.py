...
reset_points = [[0, 0]] #拠点に戻るサイクルごとの(何回目の移動で戻るか:充電の余裕)
cycle = 0
...
def detour_without_zero(pre_route):
    args1 = strtoint(pre_route) #argsから0番目の要素を消す＋int型に
    #現状の余裕を計算
    yoyuu = DRONE_BATTERY
    ...
    for i in range(len(args1) - 1):
        ...
        if args1[i + 1] == 0: 
            ...
            pass
    '''
        for j in range(len(list_time)): #全チェックポイントに経過時間を追加
            list_time[j] += moving
    ...
    for i in range(len(args1)-1): #動く回数は(長さ)−1回
        ...
        finally_path.pop(-1)
        add_route = finally_path
        detour_route.extend(add_route)
        if args1[i+1] == 0: #拠点に戻ったら次のサイクルへ
            cycle += 1
    target = 0
    detour_route = [item for item in detour_route if item != target]
    for i in range(len(detour_route)): #チェックポイント番号→座標
        detour_route[i] = nodes[detour_route[i]]
        
    return detour_route
'''
def detour(pre_route):
    args1 = strtoint(pre_route) #argsから0番目の要素を消す＋int型に
    #現状の余裕を計算
    yoyuu = DRONE_BATTERY
    #list_time = [0] * (len(nodes) - 1) #チェックポイントごとの経過時間
...
#確認用
def main():
    sample_route = ["0","9","10","18","17","23","29","19","0","12","11","24","0","28","32","30","21","22","0","20","25","14","13","16","0","8","7","6","5","4","1","27","3","2","25","0","31","26","0"]
    #sample_route = args.pop(0)
    result = detour_without_zero(sample_route)
    print(result)
